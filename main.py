...
def build_router(homebase: HomeBase, memory: MemoryStore) -> Router:
    ...
    def pc(argument: str) -> str:
        ...
        if action == "open": action = "open_app"
        elif action == "search": action = "google_search"
        elif action in {"site", "navigate", "go"}: action = "open_site"
        elif action in {"window", "win"}: action = "window"
        try:
    ...
    def files(argument: str) -> str:
        ...
        except (CapabilityError, OSError) as exc: return f"File access denied safely: {exc}"
    def read(argument: str) -> str:
        try: return read_file(argument)
        except (CapabilityError, OSError) as exc: return f"File read denied safely: {exc}"
    def web(argument: str) -> str:
        try: return fetch_web(argument)
        except (CapabilityError, OSError) as exc: return f"Web retrieval denied safely: {exc}"
    ...
    def remember(argument: str) -> str:
        if "=" not in argument: return "Use: remember key = value"
...
def handle_command(router: Router, brain: Brain, decision: DecisionEngine, context: ContextBuilder, home_control: HomeControl, command: str, events: EventBus) -> tuple[str, bool]:
    ...
    if normalized in {"exit", "quit", "goodbye", "shutdown jarvis"}:
        events.publish("assistant.state", state="idle"); return "Standing by.", False
    if normalized == "help":
        response = router.help_text() + "\n\n" + home_control.help_text(); events.publish("assistant.state", state="speaking"); return response, True
    if normalized == "home" or normalized.startswith("home "):
        response = home_control.execute(normalized.removeprefix("home").strip()); events.publish("assistant.state", state="speaking"); return response, True
    response = home_control.execute(proposal.target or "status") if proposal.action == "HOME" else router.route(brain.normalize(command))
...
def run_text(router: Router, brain: Brain, decision: DecisionEngine, context: ContextBuilder, home_control: HomeControl, homebase: HomeBase, events: EventBus) -> None:
    ...
    while True:
        ...
        except (EOFError, KeyboardInterrupt): events.publish("assistant.state", state="idle"); print("\nJ.A.R.V.I.S. > Session ended."); return
...
def run_voice(router: Router, brain: Brain, decision: DecisionEngine, context: ContextBuilder, home_control: HomeControl, homebase: HomeBase, events: EventBus) -> None:
    voice = VoiceInterface(); voice.speak(f"{homebase.assistant_name()} online. Home Base loaded."); events.publish("assistant.state", state="idle")
    while True:
        try: events.publish("assistant.state", state="listening"); command = voice.listen()
        except VoiceUnavailable as exc: print(f"Voice error: {exc}"); return
        if not command: continue
        print(f"You > {command}"); response, keep_running = handle_command(router, brain, decision, context, home_control, command, events); print(f"J.A.R.V.I.S. > {response}"); events.publish("assistant.state", state="speaking"); voice.speak(response)
        if not keep_running: return
        events.publish("assistant.state", state="idle")
...
def main() -> None:
    ...
    try: homebase = HomeBase.load(); home_control = HomeControl(homebase)
    except HomeBaseError as exc: print(f"Home Base error: {exc}"); return
    memory = MemoryStore(); router = build_router(homebase, memory); brain = Brain(); decision = DecisionEngine(brain, router); context = ContextBuilder(memory); events = EventBus()
    gateway = HomeBaseGateway(events, lambda command: handle_command(router, brain, decision, context, home_control, command, events)[0]); gateway.start(); print(f"Home Base UI bridge: {gateway.address}")
    try:
        if args.voice:
            try: run_voice(router, brain, decision, context, home_control, homebase, events)
            except VoiceUnavailable as exc: print(f"Voice mode unavailable: {exc}"); print("Falling back to text mode."); run_text(router, brain, decision, context, home_control, homebase, events)
        else: run_text(router, brain, decision, context, home_control, homebase, events)
